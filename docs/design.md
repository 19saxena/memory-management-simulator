...
## 7. Test Workloads and Logs
Sample command sequences for allocator and cache validation are provided in the `tests/` directory.  
Run them using:
mingw32-make run < tests/{filename}.txt
or
memsim.exe < tests/{filename}.txt

Sample terminal outputs validating allocation, fragmentation, buddy system, and cache hierarchy are stored in the `logs/` directory.
